...
void run_server(char *port) {
    ...
    int returned = getaddrinfo(NULL, port, &hints, &res);
    if(returned != 0) {
        const char *mesg = gai_strerror(returned);
        fprintf(stderr, "%s\n", mesg);
        freeaddrinfo(res);
        res = NULL;
        exit(1);
    ...
    pthread_t all_threads[MAX_CLIENTS];
    while(!endSession) {
        printf("Waiting for connection...\n");
        int cd = accept(serverSocket, NULL, NULL);
        if(cd == -1) {
            perror(NULL);
            exit(1);
        }
        ...
        pthread_create(all_threads+i, NULL, process_client, (void*)i);
    }
}
