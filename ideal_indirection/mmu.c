...
void mmu_read_from_virtual_address(mmu *this, addr32 virtual_address,
    ...
    if (!pte) {
        ...
        addr32 addr = (virtual_address) >> 20;
        page_directory_entry *pde = &page_dir->entries[addr];
        ...
        addr32 pte_addr = (virtual_address & 0x003FF000) >> 12;
        pte = &(((page_table*)get_system_pointer_from_pde(pde))->entries[pte_addr]);
    }
    ...
    pte->accessed = 1;
    memcpy(buffer, (void *)get_system_pointer_from_pte(pte) + (virtual_address & 0xFFF), num_bytes);
...
void mmu_write_to_virtual_address(mmu *this, addr32 virtual_address, size_t pid,
    ...
    if (!pte) {
        mmu_tlb_miss(this);
        page_directory* page_dir = this->page_directories[pid];
        addr32 pde_addr = (virtual_address) >> 20;
        page_directory_entry *pde = &page_dir->entries[pde_addr];
        if (!pde->present) {
            pde->present = true;
            mmu_raise_page_fault(this);
            pde->base_addr = (ask_kernel_for_frame(NULL) >> 12);
    ...
    memcpy((void *)get_system_pointer_from_pte(pte) + (virtual_address & 0xFFF), buffer, num_bytes);
    tlb_add_pte(&this->tlb, virtual_address & 0xFFFFF000, pte);
}
